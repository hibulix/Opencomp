<div class="page-title">
    ...
    <?php echo $this->Html->link('<i class="fa fa-arrow-left"></i> '.__('retour à la classe'), '/classrooms/viewreports/'.$report->classroom_id, array('class' => 'ontitle btn btn-default', 'escape' => false)); ?>
</div>
...
<table class="table table-striped">
	...
	<?php foreach($results as $result): ?>
		<?php $total_items = $result->sum_grade_a + $result->sum_grade_b + $result->sum_grade_c + $result->sum_grade_d; ?>
		<tr>
			<td><?php echo $result->pupil->first_name; ?></td>
		    <td><?php echo $result->pupil->name; ?></td>
		    <td class="text-center"><?php echo $result->sum_grade_a; ?></td>
		    <td class="text-center"><?php echo $result->sum_grade_b; ?></td>
		    <td class="text-center"><?php echo $result->sum_grade_c; ?></td>
		    ...
		    <div class="progress" style="margin-bottom:0px;">
				...
				<div class="info progress-bar progress-bar-danger" rel="tooltip" data-placement="bottom" title="<?php echo $this->Utils->getPercentValue($result->sum_grade_d, $total_items); ?>% des items non acquis <?php echo $result->sum_grade_d ?> D sur <?php echo $total_items ?> items évalués au total" style="width: <?php echo $this->Utils->getPercentValue($result->sum_grade_d, $total_items); ?>%;"></div>
			</div>
