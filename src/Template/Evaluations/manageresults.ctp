...
<ul class="nav nav-pills">
  <li><?php echo $this->Html->link(__('1. Définir les items évalués'), array('controller' => 'evaluations', 'action' => 'attacheditems', $evaluation->id)); ?></li>
  <li class="active"><?php echo $this->Html->link(__('2. Saisir les résultats'), array('controller' => 'evaluations', 'action' => 'manageresults', $evaluation->id)); ?></li>
...
<?php if (!empty($evaluation->items)): ?>
	<div class="page-title">
	    ...
	    <?php echo $this->Html->link('<i class="fa fa-keyboard-o"></i> '.__('saisie manuelle'), '/results/selectpupilmanual?evaluation_id='.$evaluation->id, array('class' => 'btn btn-default', 'escape' => false)); ?>
        <?php echo $this->Html->link('<i class="fa fa-magic"></i> '.__('saisie automagique'), '/results/selectpupil?evaluation_id='.$evaluation->id, array('class' => 'btn btn-primary', 'escape' => false)); ?>
        </div>
	</div>
	...
	<?php
		...
		<tr>
			...
			<td class="actions">
				<?php echo $this->Html->link('<i class="fa fa-pencil"></i> '.__('Compléter ou modifier la saisie'), array('controller' => 'results', 'action' => 'add', 'pupil_id' => $pupil->id, 'evaluation_id' => $evaluation->id, 'manual' => 'true'), array('escape' => false)); ?>
			</td>
		</tr>
	<?php endforeach; ?>
	</table>
...
<div class="alert alert-info">
    ...
    Commencez par <?php echo $this->Html->link(__('associer des items'), array('controller' => 'evaluations', 'action' => 'attacheditems', $evaluation->id)); ?> à cette évaluation.
</div>
<?php endif;
